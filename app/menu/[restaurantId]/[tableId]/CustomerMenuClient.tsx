...
type MenuItem = { id: string; category_id: string | null; name: string; description: string | null; price: number; image_url: string | null; is_veg: boolean };
...
export default function CustomerMenuClient({ restaurantId, tableId, items, categories }: { restaurantId: string; tableId: string; items: MenuItem[]; categories: Category[] }) {
  ...
  return <>
    ...
    {selectedItem && <ItemDetails item={selectedItem} quantity={cart.find((line) => line.id === selectedItem.id)?.quantity ?? 0} onClose={() => setSelectedItem(null)} onAdd={() => add(selectedItem)} onChange={(delta) => changeQuantity(selectedItem.id, delta)} onOpenCart={() => { setSelectedItem(null); setShowCart(true); }} />}
